...
def bytes_to_unicode():
    ...
    bs : ASCII 코드 0부터 255까지의 코드
    cs : ASCII 코드에 해당하는 알파벳 또는 특수문자 (러시아어, 프랑스어, 독일어 알파벳 다 포함)
    ...
    for b in range(2 ** 8):
        if b not in bs:
            bs.append(b)
            cs.append(2 ** 8 + n)  # byte string -> character string 변환
            n += 1
...
def get_pairs(word):
    ...
    pairs = set()  # 빈 집합 생성
    prev_char = word[0]  # word의 첫번째 원소
    for char in word[1:]:
        # word의 두번째부터 끝까지 하나씩
        pairs.add((prev_char, char))  # (첫번째 단어, 두번째 단어)를 튜플로 만들어서 계속 더해줌
        # bigram을 단어로 확장한 개념으로 볼 수 있음
        prev_char = char
    return pairs


def basic_clean(text):
    text = ftfy.fix_text(text)  # 깨진 글자 수정하기
    text = html.unescape(html.unescape(text))
    return text.strip()


def whitespace_clean(text):
    text = re.sub(r'\s+', ' ', text)  # 공백 없애기
    text = text.strip()
    return text
...
class SimpleTokenizer(object):
    ...
    def bpe(self, token):
        ...
        pairs = get_pairs(word)  # word를 pair로 만들기

        if not pairs:
            # 무조건 <\w> 붙이게 (토큰화라고 생각하면 될듯.. subword 구분자 정도..?)
            return token + '</w>'
